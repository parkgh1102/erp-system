    ...
    try {
      ...
      await AppDataSource.transaction(async transactionalEntityManager => {
        ...
        await transactionalEntityManager.query(
          `DELETE FROM transaction_items WHERE "transactionId" IN (SELECT id FROM transactions WHERE "businessId" = $1)`,
          [businessId]
        );
        // 2. invoices 삭제 (transactions에 의존)
        await transactionalEntityManager.query(
          `DELETE FROM invoices WHERE "businessId" = $1`,
          [businessId]
        );
        // 3. 매출 품목 삭제
        await transactionalEntityManager.query(
        ...
        await transactionalEntityManager.query(
          `DELETE FROM transactions WHERE "businessId" = $1`,
          [businessId]
        );
        // 7. 매출 삭제
        await transactionalEntityManager.query(
        ...
        try {
          ...
          logger.info('activity_logs 테이블에 businessId 컬럼이 없어 스킵합니다.');
        }
        // 12. 알림 삭제 (선택적 - businessId 컬럼이 있는 경우만)
        try {
          await transactionalEntityManager.query(
            `DELETE FROM notifications WHERE "businessId" = $1`,
            [businessId]
          );
        } catch (e) {
          // businessId 컬럼이 없는 경우 무시
          logger.info('notifications 테이블에 businessId 컬럼이 없어 스킵합니다.');
        }
      });
